// Coffre chiffré (jeton OAuth Twitch). AES-256-GCM, clé dérivée (scrypt) de
// machine+utilisateur+sel. Pur Node : fichier inutilisable sur une autre machine/compte.
...
function keyFor(saltPath) {
  let salt;
  try { salt = fs.readFileSync(saltPath); if (salt.length < 16) throw new Error('sel invalide'); }
  catch {
  ...
  const material = `${os.hostname()}|${os.userInfo().username}|snipe-twitch-v1`;
